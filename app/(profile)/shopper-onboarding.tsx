...
const categories = [
  ...
  'Automotive', 'Health', 'Food & Cooking', 'Travel', 'Photography'
];

const interests = [
  'Technology', 'Gaming', 'Fashion', 'Fitness', 'Cooking',
  'Travel', 'Photography', 'Music', 'Reading', 'Sports'
];
...
export default function ShopperOnboarding() {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Welcome, Shopper!',
        'Your shopper profile has been created successfully.',
        [{ text: 'Get Started', onPress: () => router.push('/(shopper)/sessions') }]
      );
    } catch (error) {
      Alert.alert(
        'Error',
        error instanceof Error ? error.message : 'Failed to create shopper profile'
      );
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView}>
        <View style={styles.header}>
          <Text style={styles.title}>🛍️ Become a Shopper</Text>
          <Text style={styles.subtitle}>
            Tell us about your interests to get personalized expert recommendations
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.sectionDescription}>
            What's your typical budget for expert consultations?
          </Text>
          <View style={styles.priceOptions}>
            {[
              ...
              { label: '$2.00+', min: 2.00, max: 10.00 },
            ].map((range) => (
              <TouchableOpacity
                key={range.label}
                style={[
                  styles.priceOption,
                  priceRange.min === range.min && styles.selectedPriceOption
                ]}
                onPress={() => setPriceRange({ min: range.min, max: range.max })}
              >
                <Text style={[
                  styles.priceOptionText,
                  priceRange.min === range.min && styles.selectedPriceOptionText
                ]}>
                  {range.label}
                </Text>
              </TouchableOpacity>
