...
import { paymentService, ProcessPaymentRequest, SolanaPaymentParams } from '../../services/paymentService'
...
export const verifyPayment = createAsyncThunk(
  'payments/verify',
  async (
    { transactionHash, expectedAmount }: { transactionHash: string; expectedAmount: string },
    { rejectWithValue },
  ) => {
    try {
...
export const calculateNetworkFee = createAsyncThunk('payments/calculateFee', async (_, { rejectWithValue }) => {
  try {
...
// Get SOL price
export const fetchSolPrice = createAsyncThunk('payments/fetchSolPrice', async (_, { rejectWithValue }) => {
  try {
